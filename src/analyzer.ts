...
export class Analyzer {
  ...
  private tryWords(currentTerm: string, words: string[], state0: State, finalWords: string[]): string[] {
    const nextWords = this.trie.getNextWords(currentTerm);
    const filteredWords = nextWords.filter(word => this.canBeAMatch(word, state0));
  ...
  search(term: string, maxDifferences: number) {
    ...
    return candidateWords.filter(word => this.isAMatch(word, state0)).map(elem => elem.slice(1));
  }
}
